...
const vectorNamespace = process.env.NAMESPACE || "address_v8_prod_adrc"; // address_v7_prod_adrc, address_v5_prod_adrc, address_v4_prod_adrc, address_default, addresses, name, name_address, address_v2, address_v3_adrc, address_v3_qa_adrc, address_v4_qa_adrc
...
2. Attention To Contact: Extract delivery-specific contact information, including:
   ...
   Focus only on:
   - The person responsible for receiving deliveries
...
async function fetchLineLevelMemosFromOpenAI(itemContent, apiKey) {
    ...
    try {
        const resource = 'order-vision-ai';
        // const model = (pinecone_env === "PROD") ? "gpt-4o-4" : "gpt-4o-test-4";
        // const apiVersion = '2024-08-01-preview';
        const model = (pinecone_env === "PROD") ? "o3-mini-3" : "o3-mini-test-3";
        const apiVersion = '2024-12-01-preview';

        console.log(`Using model: ${model}, resource: ${resource}`);
...
Use this JSON structure:
{
    ...
    "line_level_materials": [
        ...
        const response = await openai.chat.completions.create({
            ...
            // response_format: { type: 'json_object' }
            response_format: zodResponseFormat(LineLevelResponseSchema, "response"),
        });
...
async function main(event, callback) {
    ...
    if (!AWS) {
        const filePath = process.argv[2] || './pdfs/brazil/OR0000001631_BR_1108327.pdf';
    ...
    const [
        ...
        fetchCustomFieldsFromOpenAI(promptContent, apiKey),
    ...
    invoice.RRC ??= {};
    invoice.RRC.value = findRRC(resultInvoice.content);
    ...
    if (invoice.Items && invoice?.Items?.valueArray) {
        ...
        invoice.Items.valueArray = await Promise.all(
            invoice.Items.valueArray.map(async (item, index) => {
                ...
                if (item.content) {
                    item.RRC = findRRC(item.content.trim());
                }
                ...
                if (item?.valueObject?.Description?.content) {
                    ...
                    if (item && item.material && item.material.length > 0) {
                        const materialai = await searchMaterial(materialPineconeInitialization, embeddingOpenAI, item.material, false, false, countryKey);
                        console.log("Material AI: ", materialai);
    ...
    invoice.PDF = PDF; // send back the PDF
