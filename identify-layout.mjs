...
async function azureProcessing(PDF) {
    ...
    const modelId = process.env.DOCUMENT_INTELLIGENCE_CUSTOM_MODEL_ID || "prebuilt-layout"; // "prebuilt-invoice", "prebuilt-layout";
...
## Extract consignee name and address if available. If not available, use the ship_to (delivery) name and address.

## Extract the Sold_to name and address. Do not user vendor, that is wrong.

## Extract the contact person's info.

## Extract the Sold_to, Ship_To (delivery), and Consignee number from the street address.
...
async function main() {
    ...
    if (result) {
        ...
        const paragraphs = getParagraphs(result.paragraphs);
        try {
            let finalTables = [];
            try {
