...
const vectorNamespace = 'address_v5_prod_adrc'; // address_v4_prod_adrc, addresses, name, name_address, address_default, address_v2, address_v3_adrc, address_v3_qa_adrc, address_v3_prod_adrc
...
//     .filter(row => row[0] && row[0] <= 3000000) 
//     .map(row => {
//         if (!row[0] || row[0] > 3000000) return;
//         if (row['Central Deletion Flag'] && String(row['Central Deletion Flag']).toUpperCase() === 'X') {
//             return;
//         } // Skip deleted records

//         let countryKey = row[1] ? String(row[1]).toLowerCase() : '';
...
//             telephone1: row[9] || '',  
//             faxNumber: row[10] || '',  
//             oneTimeAccount: row[11] || '',  
//             address: row[12] ? String(row[12]).trim() : ''
//         };
//     });
