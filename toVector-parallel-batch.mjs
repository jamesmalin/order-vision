...
const sheetNameUniversalAddress = 'Sheet1';
...
const kna1Records = dataKna1
    ...
    if (row[1]) {
        ...
        countryKey = String(row[1]).toLowerCase();
    }
    let oneLineAddress = '';
    if (countryKey === 'us') {
        oneLineAddress = [
...
const records = kna1Records.map(kna1 => {
    ...
    if (!matchingUniversal) {
        ...
        if (kna1.country == 'cn' || kna1.country == 'tw' || kna1.country == 'hk' || kna1.country == 'br') {
            oneLineAddress = [
...
const chunks = (array, batchSize = 50) => {
    const chunked = [];
...
(async () => {
    ...
    // Split the vectors into chunks for parallel upserts
    const vectorChunks = chunks(vectors, batchSize);

    // Perform async parallel upserts with progress logging
    let currentBatchStart = 0;
    await Promise.all(vectorChunks.map(async (chunk, index) => {
        await upsert(pineconeIndex, vectorNamespace, chunk);
        currentBatchStart = index * batchSize + 1;
        console.log(`Upserted batch of ${chunk.length} vectors from record ${currentBatchStart} to ${currentBatchStart + chunk.length - 1}`);
    }));
